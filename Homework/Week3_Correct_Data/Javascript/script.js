...
function display(info, x_min, x_max, y_min, y_max, colors){
  key_names = info[3]

  // get transform function for y axis
  y_domain = [Math.min(...info[0]) * 1.1, Math.max(...info[1]) * 1.1];
  y_range = [y_min, y_max];
  transform_y = createTransform(y_domain, y_range);

  // get transform function for x axis
  x_domain = [0, info[3].length];
  x_range = [x_min, x_max]
  ...
  // graph title
  ctx.font = "30px Arial";
  ctx.fillText("Average, minimum and maximum temperature at the Bilt, Netherlands (2008 - 2018)",x_min, y_max * 0.9);

  // graph legend info
  legend_x = transform_x(94)
  legend_y = transform_y(35.5)
  legend_width = 200
  legend_height = 100
...
function drawlegend(legend_x, legend_y, legend_width, legend_height, ctx, colors, labels){
  ...
  for(i = 0; i < colors.length; i++){
    ...
    ctx.fillStyle = 'Black'
    ctx.font = "25px Arial";
    ctx.fillText(labels[i], legend_x + legend_width/ 3, legend_y + (legend_height/3) + i * (legend_height / 3.7))

    // ctx.fillText(String(parseInt(key_names[i-1][0])+1), transform_?x(i-1), transform_y(-2))
  }
}
...
GetInfo = function(data){
  ...
  for(i = 0; i < keys.length; i++){
    year_data = data[keys[i]]
    year_keys = Object.keys(year_data);

    // within each year over months
    for(j = 0; j < year_keys.length; j++){

      // month_keys = year_data[year_keys[j]]
